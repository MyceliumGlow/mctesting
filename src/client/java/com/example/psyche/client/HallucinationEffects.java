...
public final class HallucinationEffects {
    ...
    public static void tick(MinecraftClient client) {
        ...
        if (flickerTicks > 0) {
            ...
            if (flickerTicks == 1 && originalSlot >= 0) {
                p.getInventory().selectedSlot = originalSlot;
                originalSlot = -1;
            }
        }
    ...
    public static void triggerPhantomBreathing() {
        ...
        ClientPlayerEntity p = client.player;
        client.world.playSound(p.getX() + (RNG.nextBoolean() ? 1.5 : -1.5), p.getY(), p.getZ(),
                SoundEvents.ENTITY_PLAYER_BREATH, SoundCategory.AMBIENT, 0.05f, 0.85f, false);
    }
    ...
    public static void triggerFalseFootstep() {
        ...
        client.world.playSound(p.getX() - Math.sin(yaw) * 2.1, p.getY(), p.getZ() + Math.cos(yaw) * 2.1,
                SoundEvents.ENTITY_PLAYER_STEP, SoundCategory.AMBIENT, 0.08f, 0.6f, false);
    }
    ...
    public static void triggerInventoryFlicker() {
        ...
        ClientPlayerEntity p = client.player;
        originalSlot = p.getInventory().selectedSlot;
        int other = (originalSlot + 1 + RNG.nextInt(8)) % 9;
        p.getInventory().selectedSlot = other;
        flickerTicks = 3;
    }
    ...
    public static void triggerPeripheralShadow() {
        ...
        for (int i = 0; i < 4; i++) {
            client.world.addParticle(ParticleTypes.SMOKE, x, p.getY() + 0.1 + i * 0.18, z, 0, 0.01, 0);
        }
    }
}
